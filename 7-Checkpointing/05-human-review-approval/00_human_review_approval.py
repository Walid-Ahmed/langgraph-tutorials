...
#
# Learning sequence:
#   1. invoke(input, config) runs until the interrupt and saves a checkpoint.
#   2. Plain Python shows the saved draft and asks the human what to do.
#   3. update_state(config, decision) writes that decision into the checkpoint.
#   4. invoke(None, config) reloads the same thread and continues from the pause.
#
# The human input is intentionally collected OUTSIDE the graph. The graph is
# paused while the person reviews the draft; no graph node is waiting on input().
#
...
# This TypedDict defines the shared state that moves through the graph.
# Every node receives the complete state as a normal dictionary, reads the
# fields it needs, and returns only the fields it wants LangGraph to update.
class ApprovalState(TypedDict):
    request: str   # The user's original drafting request.
    draft: str     # The first response generated by the LLM.
    approved: bool  # The human's approve (True) or reject (False) decision.
    feedback: str  # Improvement instructions supplied after a rejection.
    final: str     # The approved draft or the LLM's revised response.
...
def main() -> None:
    ...
    # Use an absolute repository-based path so graph rendering also works when
    # an IDE launches this script with a different current working directory.
    plot_graph(
        graph,
        REPO_ROOT / "7-Checkpointing/diagrams/05_human_review_approval_graph.png",
    )

    # Every persistence operation below uses this same thread_id. That is how
    # get/update/resume operations target this particular paused execution.
